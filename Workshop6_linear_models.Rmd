...
This looks... not great? And it doesn't even have the regression line! We can add that with the `geom_smooth()` function which is used to add a variety of analysis-based lines. We need to tell it we are using the "glm" method of analysis, and specifically the binomial "family" in glm.
...
Finally, we get a line! the direction of the line, descending on the y-axis from 1 to 0 as F2 ascends, tells us that lower F2 values are associated with the points at "1" on the y-axis, and higher F2 values are associated with the points at "0". Since these are just substitutes for the `sex` categories of "male" and "female" respectively, it would be helpful to now replace these converted numbers with the original labels!
...
We can plot the distribution and the linear model together using a similar ggplot call as we used earlier for F2 by sex. In this case we have a bit less work to do because both variables, F2 and year, are numeric. We'll use `geom_jitter()` again with its default settings, and `geom_smooth()` with the "lm" method since this is a simple linear model.
