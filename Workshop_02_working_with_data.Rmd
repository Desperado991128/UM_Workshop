...
Most of the workshop, and especially anything involving making plots, will be carried out using packages from the "tidyverse":
...
Tidyverse packages build on the base R functionality in a lot of useful ways, which will be introduced one at a time as they become relevant.

---

# Data visualisation

The following section is a verbatim copy of Chapter 3 of "R for Data Science" by Garrett Grolemund & Hadley Wickham, which I have included here rather than switching over to the set of original source code files. This chapter deals with data which is not related to linguistics in any way, but the underlying methodology for plotting data is exactly the same no matter what its source.

In respect of the **Creative Commons Attribution-NonCommercial-NoDerivs 3.0 License**, I have not altered any of the code; we can skip through sections as seems appropriate as we go along. 

You can access R for Data Science in several ways, which are all free except for obtaining a printed copy.

* Website: <https://r4ds.had.co.nz>
* Source code: <https://github.com/hadley/r4ds>
* Print copy: <https://www.amazon.ca/Data-Science-Transform-Visualize-Model/dp/1491910399>

If you download the source code, you will have access to the entire book in its original Markdown format. However, because that code is formatted with the aim of generating a book, it works a little differently than the workshop files we are using, and you will not be able to knit chapters one-by-one without some modification. This is my reason for copying the source code for this chapter into the workshop rather than working with it directly.
...
```{r aesthetics, echo = FALSE, asp = 1/4}
ggplot() +
...
```{r alpha_shape, out.width = "50%", fig.align = 'default', warning = FALSE, fig.asp = 1/2, fig.cap =""}
# Left
ggplot(data = mpg) + 
...
```{r point_vs_smooth, echo = FALSE, out.width = "50%", fig.align="default", message = FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy))
...
```{r point_smooth, echo = FALSE, message = FALSE}
...
```{r group, fig.width = 3, fig.align = 'default', out.width = "33%", message = FALSE}
ggplot(data = mpg) +
  geom_smooth(mapping = aes(x = displ, y = hwy))
...
```{r multiple_geoms, message = FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) +
  geom_smooth(mapping = aes(x = displ, y = hwy))
```
...
```{r local_mappings, message = FALSE}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
...
```{r global_local_data, message = FALSE}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(mapping = aes(color = class)) + 
...
1.  Recreate the R code necessary to generate the following graphs.
    ...
    ```{r recreate, echo = FALSE, fig.width = 3, out.width = "50%", fig.align = "default", message = FALSE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
      geom_point() + 
      geom_smooth(se = FALSE)
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
...
```{r geom_bar, echo = FALSE, out.width = "100%"}
...
```{r fill, out.width = "50%", fig.align = "default"}
ggplot(data = diamonds) + 
...
*   `position = "identity"` will place each object exactly where it falls in 
    ...
    ```{r overlap, out.width = "50%", fig.align = "default"}
...
*   `coord_flip()` switches the x and y axes. This is useful (for example),
    ...
    ```{r coord_flip, fig.width = 3, out.width = "50%", fig.align = "default"}
...
*   `coord_quickmap()` sets the aspect ratio correctly for maps. This is very
    ...
    ```{r coord_quickmap, fig.width = 3, out.width = "50%", fig.align = "default", message = FALSE}
    nz <- map_data("nz")
...
*   `coord_polar()` uses polar coordinates. Polar coordinates reveal an 
    ...
    ```{r coord_polar, fig.width = 3, out.width = "50%", fig.align = "default", fig.asp = 1}
...
1.  What does the plot below tell you about the relationship between city
    ...
    ```{r geom_abline, fig.asp = 1, out.width = "50%"}
    ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
      geom_point() + 
      geom_abline() +
      coord_fixed()
...
```{r transform, echo = FALSE, out.width = "100%"}
...
```{r extend, echo = FALSE, out.width = "100%"}
